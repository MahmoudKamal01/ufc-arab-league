...
const UserRankTable = () => {
  ...
  const [userPredictionsName, setUserPredictionsName] = useState("User");
  ...
  const handleShowPredictions = async (userId, userName) => {
    setUserPredictionsName(userName);
    if (isLoggedIn) {
      try {
              ...
              {userRankData.map((user, index) => (
                ...
                >
                  ...
                  <td className="py-2 text-center">
                    <button
                      onClick={() =>
                        handleShowPredictions(user._id, user.userId.name)
                      }
